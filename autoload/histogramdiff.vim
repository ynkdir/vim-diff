...
function histogramdiff#diffexpr()
  call histogramdiff#fnormal(v:fname_in, v:fname_new, v:fname_out)
endfunction
...
function histogramdiff#fnormal(oldfile, newfile, outfile)
  let old = readfile(a:oldfile, 'b')
  let new = readfile(a:newfile, 'b')
  let out = histogramdiff#bnormal(old, new)
  if !empty(out)
    call add(out, '')
  endif
